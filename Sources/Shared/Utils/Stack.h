...
public:
    ...
    // TODO: Unify the walk functions somehow
    template <typename Function>
    void walk(Function function)
    {
        for (uint32_t index = 0; index < size(); index++) {
            function(m_stack[size() - index - 1]);
        }
    }

    template <typename Value, typename Function>
    [[nodiscard]] std::optional<Value> walk_for_value(Function function)
    {
        for (uint32_t index = 0; index < size(); index++) {
            auto maybe_value = function(m_stack[size() - index - 1]);
            if (maybe_value)
                return maybe_value;
        }
        return {};
