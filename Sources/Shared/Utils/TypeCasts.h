...
template <typename OutputType, typename InputType>
ALWAYS_INLINE bool is(std::shared_ptr<InputType> input)
{
...
template <typename OutputType, typename InputType>
ALWAYS_INLINE CopyConst<InputType, OutputType>* as_if(std::shared_ptr<InputType> input)
{
    if (!input)
        return nullptr;
    return as_if<OutputType>(*input);
...
template <typename OutputType, typename InputType>
ALWAYS_INLINE CopyConst<InputType, OutputType>* as(std::shared_ptr<InputType> input)
{
    if (!input)
        return nullptr;
    auto* result = as_if<OutputType>(input);
